...
from algorithm.utils.mlp import MLPBase
from algorithm.utils.mappo_util import init_rnn, init_module
from algorithm.utils.encoder_decoder import build_encoder_inputs
...
class RNNPOAMAgent(nn.Module):
    ...
    def _build_inputs(self, batch, t=None):
        ...
        inputs = []
        inputs.append(batch["obs"][:, ts].expand(bs, max_t, self.n_agents, -1) if t is None \
                else batch["obs"][:, ts].expand(bs, 1, self.n_agents, -1))  # b1av
        if self.args.obs_last_action:
            if t is None: # build last act for all timesteps
                ...
                # last_act_all = last_act_all.repeat(1, 1, self.n_agents)
                last_act_all.shape
