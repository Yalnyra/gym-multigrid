...
from algorithm.modules.mixers.vdn import VDNMixer
from algorithm.modules.mixers.qmix import QMixer
...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        chosen_action_qvals = th.gather(
            mac_out[:, :-1], 
            dim=-1, 
            index=actions.unsqueeze(-1)).squeeze(
            -1
        )  # Remove the last dim
        ...
        for t in range(batch.max_seq_length):
            ...
            if (isinstance(target_agent_outs, tuple)):
                target_agent_outs, _ = target_agent_outs
